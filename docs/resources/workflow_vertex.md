...
page_title: "stonebranch_workflow_vertex Resource - stonebranch"
subcategory: ""
description: |-
  Manages a task (vertex) within a StoneBranch Workflow. Use this resource to add existing tasks to a workflow.
---
...
- `task_name` (String) Name of the task to add to the workflow.
- `workflow_name` (String) Name of the workflow to add the task to.

### Optional

- `alias` (String) Alias for this task instance in the workflow. Useful when the same task appears multiple times.
- `vertex_x` (String) X coordinate for the task position in the workflow diagram.
- `vertex_y` (String) Y coordinate for the task position in the workflow diagram.

### Read-Only

- `vertex_id` (String) Unique vertex ID assigned by StoneBranch. Used to identify this task instance within the workflow.
